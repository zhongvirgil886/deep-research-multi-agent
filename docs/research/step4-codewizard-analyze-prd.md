...
##### 12.3.2.1 接续 Analyze 阶段：在 DataAnalyst 后执行代码增强分析

**当前行为**：Graph 在 DataAnalyst 后调用 `CodeWizard.process(state)`。

**目标**：在结构化数据和初步图表生成之后，尝试用 Python 代码进一步完成数据分析或图片图表生成。

**作用**：DataAnalyst 更偏“抽取和结构化”，CodeWizard 更偏“可执行分析和图表产物”。两者共同组成 Analyze 阶段。

**当前限制**：当前 CodeWizard 仍依赖同一个累计 `ResearchState`，没有明确区分哪些数据来自 Step2、Step3 或补充搜索。

##### 12.3.2.2 检查数据点数量：判断是否具备最低分析条件

**当前行为**：CodeWizard 检查 `data_points` 数量；本例为 3 条，允许进入分析。

**目标**：避免在数据太少时生成没有意义或误导性的分析代码和图表。

**作用**：这是最低执行门槛。数据点不足时应跳过代码执行，避免 LLM 为了完成任务而编造数据。

**当前限制**：数量达标不代表数据质量达标。3 条数据点可能单位不同、年份不同、口径不同，仍不适合直接分析。后续应检查单位、年份、指标类型和来源一致性。

##### 12.3.2.3 生成分析代码：把数据摘要交给 LLM 形成 Python 脚本

**当前行为**：`_analyze_data()` 将数据点格式化后，通过 `ANALYSIS_PROMPT` 调用 LLM 生成 Python 代码。

**目标**：让 LLM 根据当前数据点生成可执行的分析和绘图代码。

**作用**：这是当前 CodeWizard 的核心生成动作。它把结构化数据点转为 pandas/matplotlib 等代码，尝试产出图表和分析结果。

**当前限制**：当前是 LLM 直接生成代码，不是从预置函数库中选择。分析逻辑、计算方式和图表选择都可能不稳定，生产化应改为 `AnalysisPlan -> 确定性函数 -> 图表模板`。

##### 12.3.2.4 清理代码文本：把 LLM 输出转成可编译 Python

**当前行为**：`_clean_code()` 清理 Markdown 代码块、转义换行、错误续行符和粘连语句。

**目标**：处理 LLM 常见输出格式问题，让代码更接近可执行 Python。

**作用**：这是执行前的格式修复层。它可以清理 ```python 代码块、错误换行和部分粘连语句，降低语法失败概率。

**当前限制**：代码清理只能修复文本格式，不保证分析逻辑正确，也不能保证代码安全。

##### 12.3.2.5 语法预检查：在沙箱执行前拦截明显错误

**当前行为**：`_execute_code()` 先用 `compile()` 做语法预检查。

**目标**：在真正执行代码前发现语法错误，避免无意义地进入沙箱运行。

**作用**：这是低成本的失败短路机制。语法错误会提前返回，减少运行时异常和调试成本。

**当前限制**：`compile()` 只能检查语法，不能发现运行时错误、错误计算、错误图表或安全风险。

##### 12.3.2.6 安全模式检查：拦截明显危险代码

**当前行为**：`_is_code_safe()` 检查危险模式，如 `open()`、`requests`、`eval()`。

**目标**：阻止 LLM 生成的代码访问文件、网络、动态执行或其他高风险能力。

**作用**：这是当前沙箱前的安全门。它能挡住一部分明显危险模式，降低执行 LLM 代码的风险。

**当前限制**：当前主要是正则黑名单，不是完整安全沙箱。黑名单可能漏掉变体写法，也不能证明代码逻辑安全。生产化应使用容器隔离、资源限制和白名单 API。

##### 12.3.2.7 沙箱执行代码：在受限环境中运行分析脚本

**当前行为**：`_execute_in_sandbox()` 在线程中运行受限环境，预置 `pd/np/plt/sns`。

**目标**：让通过检查的代码在受限上下文中执行，生成分析结果或 Matplotlib 图表。

**作用**：这是 CodeWizard 从“生成代码”到“生成可视化产物”的执行环节。预置 pandas、numpy、matplotlib、seaborn 可以支持常见数据分析和绘图。

**当前限制**：线程内受限执行不是强隔离。它不能等同于安全容器，也缺少严格 CPU、内存、文件系统和网络隔离。

##### 12.3.2.8 自愈修复重试：在执行失败时尝试自动修复代码

**当前行为**：如果执行失败，`_execute_with_self_correction()` 调用 `_fix_code()`，最多重试 3 次。

**目标**：让系统在常见代码错误发生时自动请求 LLM 修复，而不是直接失败。

**作用**：这是 Demo 稳定性增强机制。它可以处理部分变量名错误、库调用错误、格式错误或简单逻辑问题。

**当前限制**：自愈重试仍依赖 LLM，可能越修越偏，也可能掩盖原始问题。对于严肃分析，不应依赖 LLM 反复修代码来保证结果正确。

##### 12.3.2.9 捕获图表产物：把 Matplotlib 输出写入状态

**当前行为**：生成 Matplotlib figure 后，系统把 PNG 捕获为 base64，追加到 `state["charts"]`。

**目标**：把代码执行得到的图片图表转为前端可展示、报告可引用的状态数据。

**作用**：这是 CodeWizard 对外交付的主要产物之一。`image_base64` 可以被前端图表区渲染，也可以被 LeadWriter 在报告中引用。

**当前限制**：当前图表写入主要保存图片结果，缺少 `source_data_point_ids`、计算公式、数据一致性校验和图表类型规则。后续应确保每个图表数字都能回溯到输入数据点或确定性计算结果。
